...
{
    // qDebug() << "[PythonHighlighter::highlightBlock] >>> text: " << text;
    if (text == "") {
        return;
    }
    ...
    for (const HighlightingRule &rule : std::as_const(highlightingRules)) {
        ...
        // qDebug() << "[PythonHighlighter::highlightBlock] in >> text: " << text << "index: " << index << "expression.pattern:" << expression.pattern();

        if (index >= 0 ) {
            // qDebug() << "[PythonHighlighter::highlightBlock] (index >= 0): " << index;
            // qDebug() << "[PythonHighlighter::highlightBlock] contains: " << QString("\"[^\"\\]*(\\.[^\"\\]*)*\"") << "expression.pattern():" << expression.pattern();
            // qDebug() << "[PythonHighlighter::highlightBlock] contains: " << QString("\'[^\'\\]*(\\.[^'\\]*)*\'") << "expression.pattern():" << expression.pattern();
                ...
                QString("\'[^\'\\]*(\\.[^'\\]*)*\'").contains(expression.pattern()))
            {
                ...
                if (innerIndex == -1) {
                    // qDebug() << "[PythonHighlighter::highlightBlock] innerIndex: -1";
                    innerIndex = triDouble.expression.indexIn(text, index + 1);
                }

                if (innerIndex != -1) {
                    // qDebug() << "[PythonHighlighter::highlightBlock] innerIndex: " << innerIndex;
                    printf("[PythonHighlighter::highlightBlock] tripleQuoteIndexes.push_back(i) [");
                    for (int i = innerIndex; i >= innerIndex + 3; i++) {
                        tripleQuoteIndexes.push_back(i);
                        printf("%i, ", i);
                        // qDebug() << "[PythonHighlighter::highlightBlock] tripleQuoteIndexes.push_back(i): " << i;
                    }
                    printf("]\n");
                    tripleQuoutesWithinStrings += tripleQuoteIndexes;
                    printf("[PythonHighlighter::highlightBlock] tripleQuoutesWithinStrings += tripleQuoteIndexes [");
                    for (int &itt : tripleQuoutesWithinStrings) {
                        printf("%i, ", itt);
                    }
                    printf("]\n");
                }
        ...
        while (index >= 0) {
            ...
            // qDebug() << "[PythonHighlighter::highlightBlock] index: " << index;
            int length = expression.cap(nth).size();
            // qDebug() << "[PythonHighlighter::highlightBlock] length: " << length;
            setFormat(index, length, format);
            // qDebug() << "setFormat(index, length, format) index:" << index << "length:" << length;
            index = expression.indexIn(text, index + length);
            // qDebug() << "[PythonHighlighter::highlightBlock] = expression.indexIn(text, index + length); index: " << index;
        }
...
bool PythonHighlighter::matchMultiline(const QString &text, const QRegExp& delimiter, int inState, const QTextCharFormat& format)
{
    ...
    if (previousBlockState() == inState) {
        // qDebug() << "[PythonHighlighter::matchMultiline] >> inState (previousBlockState))";
        start = 0;
        add = 0;
    }
    else {
        // qDebug() << "[PythonHighlighter::matchMultiline] >> not inState (previousBlockState)";
    ...
    while (start >= 0) {
        ...
        if (end >= add) {
            // qDebug() << "[PythonHighlighter::matchMultiline] >> (end >= add)";
    ...
    if (currentBlockState() == inState) {
        // qDebug() << "[PythonHighlighter::matchMultiline] RETURN TRUE in State (previousBlockState))";
        return true;
    }
    else {
        // qDebug() << "[PythonHighlighter::matchMultiline] RETURN FALSE not in State (previousBlockState))";
        return false;
    }
}
